...
exports.handler = async function(event) {
  try {
    ...
    if(!name || !category || isNaN(Number(price))) return { statusCode: 400, body: JSON.stringify({ error: 'Invalid payload' }) };
    ...
    const newProd = { id: 'p' + Date.now(), name: String(name), category: String(category), price: Number(price), image: image && String(image).trim() ? String(image).trim() : 'https://via.placeholder.com/400x300?text=Product' };
    arr.push(newProd);
    await putFile('kasir/products.json', JSON.stringify(arr, null, 2), 'Add product from UI');
    await putFile('kasir/products.js', 'const PRODUCTS = ' + JSON.stringify(arr, null, 2) + '\n', 'Update products.js from UI');
    return { statusCode: 200, body: JSON.stringify({ ok: true, product: newProd }) };
  } catch (err) {
